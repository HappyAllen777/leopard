...
<title>编辑分类</title>
<link rel="stylesheet" href="${project}/static/oss/amazeui/amazeui.min.css">ble=no">
</head>
...
<body style="width:450px;">
	<div class="am-form" style="width: 450px;box-shadow:2px 3px 15px rgba(0,0,0,0.3);float:left">
		<fieldset>
			<legend>创建分类</legend>
			<input type="hidden" name="id" value="${baseModel.id}" fd="true" name="id" dt="text"/>
			<div class="am-form-group">
				<label>名称</label> <input type="text" class="" placeholder="输入分类名称" name="name" value="${baseModel.name}" fd="true" dt="text" minl="1" maxl="64" tip="分类名称" />
			</div>


			<div class="am-form-group">
				<label>备注</label> <textarea class="" rows="5" name="remarks" fd="true" dt="null" minl="1" maxl="128" tip="备注">${baseModel.remarks}</textarea>
			</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		function addCategory() {
			var form = formCheck();
			if (form) {
				formSubmit(form, "${project}/category/update", function(){
					showInfo("保存成功");
					setTimeout(function(){
						history.go(-1);
					},1000);
