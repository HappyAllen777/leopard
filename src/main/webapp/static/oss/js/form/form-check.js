/**
 * @author Allen 表单封装工具类,当表单数据都合乎条件,则返回表单的json对象,否则返回null 此工具并不适合表单会动态变化的场景
 * 
 * <pre>
...
function formCheck(marker) {
	...
	// 收集表单元素
	// marker = marker||"true";
	if (typeof (marker) == undefined || marker == undefined || marker == null
			|| marker.length == 0) {
		marker = "true";
	}
...
// 通用提交表单方法
function formSubmit(form, postUrl, successCallBack, failedCallBack,
		errorCallBack, arrayHandler) {
	if (formSubmitted) {
			...
			success : function(obj) {
				...
				if (obj && obj.errCode == 0) {
					if (successCallBack) {
						...
						showInfo(obj.errMsg, 5);
					}
				} else if (obj && obj.errCode != 0) {
					if (failedCallBack) {
						failedCallBack(obj);
					} else {
						showInfo(obj.errMsg, 5);
...
var getAjaxDataStatus = false;
function getAjaxData(dataUrl, successCallBack, failedCallBack,
		errorCallBack, handler) {
	if (getAjaxDataStatus) {
		showInfo("正在提交,请勿重复提交...");
		return;
	}
	getAjaxDataStatus = true;
	setTimeout(function() {
		$.ajax({
			type : "POST",
			dataType : "json",
